...
export default async function EditarContactoPage({ params, searchParams }: Props) {
  ...
  const [{ data: client }, { data: contact }] = await Promise.all([
    supabase.from("clients").select("id, display_name").eq("id", id).maybeSingle(),
    supabase
  ...
  const actualizarConIds = actualizarContacto.bind(null, id, contactId);
